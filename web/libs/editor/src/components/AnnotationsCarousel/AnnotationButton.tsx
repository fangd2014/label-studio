...
function AnnotationButtonTooltip({
  ...
  const statusBadge = useMemo(() => {
    ...
    if (acceptedState) {
      switch (acceptedState) {
        case "accepted":
          return { label: "已通过", variant: "positive" as const };
        case "rejected":
  ...
  const tooltipBadges = useMemo(() => {
    ...
    if (isGroundTruth) badges.push({ label: "真值", variant: "warning" });
    return badges;
    ...
    ({
      ...
      const deleteAnnotation = useCallback(() => {
        ...
        confirm({
          ...
          body: (
            <>
              该操作将<strong>删除所有已存在区域</strong>。确认删除吗？
              <br />
              此操作不可撤销。
            </>
          ),
      ...
      const actions = useMemo<ContextMenuAction[]>(
        () => [
          ...
          {
            label: `${isGroundTruth ? "取消设为" : "设为"}真值`,
