...
const I18N = {
  importTitle: /Import data to get your project started|导入数据以开始项目/,
  importDescription: /Connect your cloud storage or upload files from your computer|连接云存储或从本地电脑上传文件/,
  filterTitle: /No tasks found|未找到任务/,
  filterDescription: /Try adjusting or clearing the filters to see more results|请尝试调整或清空筛选条件以查看更多结果/,
  reviewerTitle: /No tasks available for review or labeling|暂无可审核或可标注任务/,
  tasksImportedHint: /Tasks imported to this project will appear here|导入到该项目的任务会显示在这里/,
  annotatorStartTitle: /Start labeling tasks|开始标注任务/,
  annotatorLabeledHint: /Tasks you've labeled will appear here|你已标注的任务会显示在这里/,
  noTasksTitle: /No tasks available|暂无任务/,
  assignedHint: /Tasks assigned to you will appear here|分配给你的任务会显示在这里/,
  availableHint: /Tasks will appear here when they become available|有可用任务时会显示在这里/,
  newTabHint: /\(opens in a new tab\)|（新标签页打开）/,
  connectButton: /Connect Cloud Storage|连接云存储/,
  importButton: /Import|导入/,
  clearFiltersButton: /Clear Filters|清空筛选/,
  labelAllButton: /Label All Tasks|标注全部任务/,
  s3: /Amazon S3(\sStorage| 存储)?/,
  gcs: /Google Cloud Storage(\sStorage| 存储)?/,
  azure: /Azure Blob(\sStorage| 存储)?/,
  redis: /Redis(\sStorage| 存储)?/,
};
...
describe("EmptyState Component", () => {
  ...
  describe("Basic Import Functionality", () => {
    it("should render the default import state when no role is specified", () => {
      ...
      expect(screen.getByText(I18N.importTitle)).toBeInTheDocument();
      expect(screen.getByText(I18N.importDescription)).toBeInTheDocument();
  ...
  describe("Role-Based Empty States", () => {
    describe("Filter-based Empty State", () => {
      it("should render filter empty state when hasFilters is true", () => {
        ...
        expect(screen.getByText(I18N.filterTitle)).toBeInTheDocument();
        expect(screen.getByText(I18N.filterDescription)).toBeInTheDocument();
    ...
    describe("Reviewer Role", () => {
      it("should render reviewer empty state", () => {
        ...
        expect(screen.getByText(I18N.reviewerTitle)).toBeInTheDocument();
        expect(screen.getByText(I18N.tasksImportedHint)).toBeInTheDocument();
    ...
    describe("Annotator Role", () => {
      it("should render annotator auto-distribution state with Label All Tasks button", () => {
        ...
        expect(screen.getByText(I18N.annotatorStartTitle)).toBeInTheDocument();
        expect(screen.getByText(I18N.annotatorLabeledHint)).toBeInTheDocument();
      ...
      it("should render annotator manual distribution state without button", () => {
        ...
        expect(screen.getByText(I18N.noTasksTitle)).toBeInTheDocument();
        expect(screen.getByText(I18N.assignedHint)).toBeInTheDocument();
      ...
      it("should render fallback annotator state for unknown distribution setting", () => {
        ...
        expect(screen.getByText(I18N.noTasksTitle)).toBeInTheDocument();
        expect(screen.getByText(I18N.availableHint)).toBeInTheDocument();
        expect(screen.getByTestId("icon-inbox")).toBeInTheDocument();
      });
    });
  });
  ...
  describe("Accessibility", () => {
    it("should have proper ARIA attributes", () => {
      ...
      const label = screen.getByTestId("empty-state-label");
      const title = screen.getByText(I18N.importTitle);
      const description = screen.getByText(I18N.importDescription);
    ...
    it("should render documentation link with proper accessibility", () => {
      ...
      expect(srText).toHaveTextContent(I18N.newTabHint);
    });
  });
  ...
  describe("Storage Provider Icons", () => {
    it("should render storage provider icons with proper tooltips", () => {
      ...
      const s3Container = screen.getByLabelText(I18N.s3);
      const gcsContainer = screen.getByLabelText(I18N.gcs);
      const azureContainer = screen.getByLabelText(I18N.azure);
      const redisContainer = screen.getByLabelText(I18N.redis);
    ...
    it("should show storage icons in correct order", () => {
      ...
      expect(iconContainers[0]).toHaveAttribute("aria-label", expect.stringMatching(I18N.s3));
      expect(iconContainers[1]).toHaveAttribute("aria-label", expect.stringMatching(I18N.gcs));
      expect(iconContainers[2]).toHaveAttribute("aria-label", expect.stringMatching(I18N.azure));
      expect(iconContainers[3]).toHaveAttribute("aria-label", expect.stringMatching(I18N.redis));
    });
  });
  ...
  describe("Button States and Props", () => {
    it("should render buttons with correct text content", () => {
      ...
      expect(screen.getByTestId("dm-connect-source-storage-button")).toHaveTextContent(I18N.connectButton);
      expect(screen.getByTestId("dm-import-button")).toHaveTextContent(I18N.importButton);
    });

    it("should render Clear Filters button with correct text", () => {
      render(<EmptyState {...defaultProps} hasFilters={true} onClearFilters={jest.fn()} />);

      expect(screen.getByTestId("dm-clear-filters-button")).toHaveTextContent(I18N.clearFiltersButton);
    });
    ...
    it("should render Label All Tasks button with correct text and state", () => {
      ...
      const labelButton = screen.getByTestId("dm-label-all-tasks-button");
      expect(labelButton).toHaveTextContent(I18N.labelAllButton);
